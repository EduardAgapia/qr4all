...
class _MenuScreenState extends State<MenuScreen> {
  ...
  static const LatLng codriiPascanilorLL = LatLng(47.280941, 26.615692);
  static const LatLng belcestiFocuriLL = LatLng(47.338233, 27.159748);
  static const LatLng dealurileBohotinuluiLL = LatLng(46.947622, 27.986547);
  static const LatLng rediuPrajeniLL = LatLng(47.401060, 27.272148);
  static const LatLng siretMoldovaLL = LatLng(47.346522, 26.827235);
  static const LatLng stefanLL = LatLng(46.885186, 27.678074);
  static const LatLng stejariiLL = LatLng(47.119188, 27.271016);
  static const LatLng valeaPrutuluiLL = LatLng(47.503098, 27.363956);
  static const LatLng iasiLL = LatLng(46.986224, 27.438754);
  ...
  String getGalCode() {
    ...
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(iasi))) {
      return 'ci';
    }
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(prut))) {
      return 'vp';
    }
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(stejari))) {
      return 'sa';
    }
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(stefan))) {
      return 'scm';
    }
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(siretMoldova))) {
      return 'sm';
    }
    if (Poly.isPointInPolygon(currentPoint, mapPolyToPointFor(rediuPrajeni))) {
      return 'rp';
    }
    if (Poly.isPointInPolygon(
        currentPoint, mapPolyToPointFor(dealurileBohotinului))) {
      return 'db';
    }
    if (Poly.isPointInPolygon(
        currentPoint, mapPolyToPointFor(codriiPascanilor))) {
      return 'cp';
    }
    if (Poly.isPointInPolygon(
        currentPoint, mapPolyToPointFor(belcestiFocuri))) {
      return 'bf';
    }
      ...
      Polygon(
          ...
          onTap: () {
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => ZoneScreen(
                    gal: getGal("Dealurile Bohotinului"),
                    galCode: 'db',
                    language: isRo ? 'ro' : 'en'),
              ),
            );
  ...
  List<Point> mapPolyToPointFor(List<LatLng> location) {
    return location.map((e) => Point(x: e.latitude, y: e.longitude)).toList();
  }
}
